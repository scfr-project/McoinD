...
void QRCodeDialog::setModel(OptionsModel *model)
{
    ...
    // update the display unit, to not use the default ("SCF")
    updateDisplayUnit();
}
...
{
    QString ret = QString("mcoin:%1").arg(address);
    int paramCount = 0;
    ...
    if (ui->chkReqPayment->isChecked())
    {
        ...
        {
            // even if we allow a non SCF unit input in lnReqAmount, we generate the URI with SCF as unit (as defined in BIP21)
            ret += QString("?amount=%1").arg(BitcoinUnits::format(BitcoinUnits::SCF, ui->lnReqAmount->value()));
            paramCount++;
        }
        else
        {
